"""
Bidirectional sync script using new architecture.

Loads configuration, creates bisync manager, and syncs all configured
folder pairs bidirectionally.

Uses RcloneOutputAnalyzer for automatic report generation.

Exit codes:
    0: Bisync completed (check report for details)
...
def main() -> int:
    """
    Run bidirectional sync workflow.

    Returns:
        Exit code (0 = success, 1 = failure)
    """
    logger.info("=" * 70)
    logger.info("BIDIRECTIONAL SYNC - Production Run")
    logger.info("=" * 70)
    ...
    try:
        ...
        logger.info(f"Folder pairs: {len(bisync_config.folders)}")
        logger.info(f"Conflict resolution: {bisync_config.conflict_resolve}")
        logger.info(f"Log directory: {common_config.log_dir}")

        if bisync_config.force:
            logger.warning("⚠️  FORCE MODE - Will sync even with many changes")
            logger.warning("⚠️  Safety checks bypassed!")
        ...
        for i, folder_pair in enumerate(bisync_config.folders, 1):
            logger.info(f"\nValidating pair {i}/{len(bisync_config.folders)}:")
        ...
        if validation_failed:
            ...
            logger.error("=" * 70)
            logger.error("Fix the issues above before running bisync.")
            return 1
        ...
        # Bisync all folders (report auto-generated)
        logger.info("")
        logger.info("=" * 70)
        logger.info("Syncing all configured folder pairs (STREAM MODE)...")
        logger.info("=" * 70)
        logger.info("")

        bisync_manager.bisync_all_stream()

        logger.info("")
        logger.info("=" * 70)
        logger.info("BISYNC COMPLETED")
        logger.info("=" * 70)
        
        # Read latest analysis report
        log_dir = Path(common_config.log_dir)
        analysis_files = sorted(log_dir.glob("*_bisync_batch_analysis.json"))
